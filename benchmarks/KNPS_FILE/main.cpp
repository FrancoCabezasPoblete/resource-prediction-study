...
void RunKnapsackExample(std::ifstream& file) {
  // Instantiate the solver.
	...
	while (std::getline(file, line)) {
		...
		// if (line.find("knapPI") != std::string::npos) {
		// 	std::printf("knapPI: %s\n", line.c_str());
		// }

		if (
			line.find("knapPI") != std::string::npos ||
	...
	int64_t total_weights = std::accumulate(
		packed_weights.begin(),
		packed_weights.end(),
		int64_t{0}
	);
...
int main(int argc, char** argv) {
    ...
    int counter = 0;
    while(counter < 100){
		operations_research::RunKnapsackExample(file);
		counter++;
    }
	return EXIT_SUCCESS;
}